...
const CategoryRegistrationPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlert({ type: 'error', message: 'Erro ao cadastrar categoria. Verifique os dados inseridos.' });
    }
  };

  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/coolmeia/categorias/${id}`, {
        method: 'DELETE',
      });
      ...
      setAlert({ type: 'error', message: 'Erro ao excluir categoria. Tente novamente.' });
    }
  };
    ...
    <div className="min-h-screen bg-amber-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="mb-8">
          ...
          <button
            type="submit"
            className="mt-4 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500"
          >
            Cadastrar Categoria
          </button>
        </form>
        ...
        <ul className="space-y-4">
          {categories.map((category) => (
            <li key={category.id} className="flex items-center justify-between bg-amber-200 px-4 py-2 rounded-md">
